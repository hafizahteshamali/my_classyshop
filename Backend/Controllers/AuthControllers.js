...
export const SignupControllers = async (req, res) => {
    try {
        ...
        return res.status(200).send({ success: true, message: "user registered successfully, otp sent to via email", user: { firstname: newUser.firstname, lastname: newUser.lastname, email: newUser.email, profile: newUser.profile, isVerified: newUser.isVerified } })
    } catch (error) {
...
export const VerifySignupControllers = async (req, res) => {
    try {
        ...
        if (userRecord.otp !== otp || userRecord.purpose !== purpose) {
            return res.status(400).send({ success: false, message: "invalid otp or purpose" })
        }
        ...
        return res.status(200).send({ success: true, message: "otp verified successfully", user: { firstname: user.firstname, lastname: user.lastname, email: user.email, profile: user.profile, isVerified: user.isVerified } });
...
export const LoginControllers = async (req, res) => {
    try {
        ...
        if (!foundUser.isVerified) {
            return res.status(400).send({ success: false, message: "user is not verified, please verify yourself" });
        }
        ...
        return res.status(200).send({ success: true, message: "user logged in successfully", user: { firstname: foundUser.firstname, lastname: foundUser.lastname, email: foundUser.email, profile: foundUser.profile, token } })
    } catch (error) {
        return res.status(500).send({ success: false, message: error.message })
    }
}
...
export const ForgotPassControllers = async (req, res) => {
    try {
        ...
        return res.status(200).send({ success: true, message: "otp sent to via email", user: { email: email } })
    } catch (error) {
        return res.status(500).send({ success: false, message: error.message })
    }
}
...
export const ResetPassControllers = async (req, res) => {
    try {
        ...
        return res.status(200).send({success: true, message: "reset password successfully", user:{firstname: userFound.firstname, lastname: userFound.lastname, password: userFound.password, profile: userFound.profile}})
    } catch (error) {
        return res.status(500).send({ success: false, message: error.message })
    }
}
